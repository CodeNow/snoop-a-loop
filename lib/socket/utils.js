...
module.exports = class SocketUtils {
  ...
  static testTerminal (instance, command, regex) {
    let socket = SocketUtils.createSocketConnection(opts.API_SOCKET_SERVER)
    let container = instance.attrs.container
    let testTerminal = SocketUtils.createTestTerminal(socket, container, command || 'sleep 1 && ping -c 1 localhost\n', regex || /from.*127.0.0.1/i)
